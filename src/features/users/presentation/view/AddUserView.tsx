...
import { AddUserSectionLabel }    from '../components/AddUserSectionLabel'
import { ROLES }                  from '../constants/roles.constants'
import { EMAIL_DOMAINS }          from '../constants/email-domains.constants'
import { ADD_USER_STYLES, ADD_USER_INPUT_STYLE, ADD_USER_LABEL_STYLE } from '../constants/add-user-styles.constants'
    ...
    <motion.div variants={pageVariants} initial="hidden" animate="visible" style={{ minHeight: 'calc(100vh - 3.5rem)', backgroundColor: '#0A0A0B', padding: '40px 48px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ padding: '28px 32px', borderRadius: 16, backgroundColor: '#111113', border: '1px solid #1F1F22' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                <label style={ADD_USER_LABEL_STYLE}>Nombre</label>
                <input className="add-user-input" placeholder="Juan" value={form.name}
                  onChange={e => set('name', e.target.value)} style={ADD_USER_INPUT_STYLE} />
              </div>
              <div>
                <label style={ADD_USER_LABEL_STYLE}>Apellido</label>
                <input className="add-user-input" placeholder="Pérez" value={form.last_name}
                  onChange={e => set('last_name', e.target.value)} style={ADD_USER_INPUT_STYLE} />
              </div>
            </div>

            <div>
              <label style={ADD_USER_LABEL_STYLE}>Correo electrónico</label>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <input
                  ...
                  style={{ ...ADD_USER_INPUT_STYLE, flex: 1 }}
                />
        ...
        <div style={{ padding: '28px 32px', borderRadius: 16, backgroundColor: '#111113', border: '1px solid #1F1F22' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              <label style={ADD_USER_LABEL_STYLE}>Contraseña</label>
              <div style={{ position: 'relative' }}>
                <input className="add-user-input" type={showPassword ? 'text' : 'password'} placeholder="••••••••"
                  value={form.password} onChange={e => set('password', e.target.value)}
                  style={{ ...ADD_USER_INPUT_STYLE, paddingRight: 40 }} />
            ...
            <div>
              <label style={{ ...ADD_USER_LABEL_STYLE, color: mismatch ? '#F43F5E' : '#71717A' }}>Confirmar contraseña</label>
              <div style={{ position: 'relative' }}>
                <input className="add-user-input" type={showConfirm ? 'text' : 'password'} placeholder="••••••••"
                  value={form.confirm} onChange={e => set('confirm', e.target.value)}
                  style={{ ...ADD_USER_INPUT_STYLE, paddingRight: 40, borderColor: mismatch ? '#F43F5E40' : '#2A2A2D' }} />
