...
const pages: ScrollPage[] = [
        ...
        <div className="flex flex-col gap-2 sm:gap-3 text-left w-full">
          ...
          <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
            {[
              ...
              <div key={label} className="rounded-lg sm:rounded-xl p-1.5 sm:p-2.5" style={{ background: 'rgba(0,0,0,0.25)', border: `1px solid ${color}30` }}>
                <p className="text-[10px] sm:text-xs font-bold" style={{ color }}>{label}</p>
                <p className="text-[9px] sm:text-[11px] text-white/50 mt-0.5 leading-snug">{sub}</p>
              </div>
            ))}
          </div>
          <Link to="/planes"
            className="inline-flex items-center justify-center gap-2 self-start px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold text-black transition-transform hover:scale-[1.02]"
            style={{ background: '#22C55E' }}>
            Ver planes
            <ArrowRight className="w-3.5 h-3.5" />
          </Link>
          <p className="text-white/25 text-[10px] sm:text-xs uppercase tracking-widest">↓ Desplaza para explorar</p>
        </div>
      ),
    },
  },

  {
    ...
    leftContent: {
      ...
      description: (
        <div className="flex flex-col gap-2 sm:gap-3 text-left w-full">
          <p className="text-[clamp(1.2rem,3.5vw,3.5rem)] font-black leading-none tracking-tighter text-white">
            Kit físico<br /><span className="text-amber-400">$600–$900</span><br />USD
          </p>
          <p className="text-white/50 text-[11px] sm:text-sm leading-relaxed">
          ...
          <div className="flex flex-col gap-1 sm:gap-1.5">
            {['pH · Temperatura · Alcohol', 'Turbidez · Conductividad', 'Bomba y motor a pasos', 'Alternativa a equipos de $15K+'].map((f, i) => (
              <div key={i} className="flex items-center gap-1.5 sm:gap-2 text-[11px] sm:text-sm text-white/55">
        ...
        <div className="flex flex-col gap-2 sm:gap-3 text-left w-full">
          ...
          <div className="grid grid-cols-1 gap-1.5 sm:gap-2">
            {[
              ...
              <div key={title} className="flex items-center gap-2 sm:gap-3 p-2 sm:p-2.5 rounded-lg sm:rounded-xl"
                style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.08)' }}>
                <Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white/50 flex-shrink-0" />
                <span className="text-[11px] sm:text-sm text-white/70">{title}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:gap-3 text-left w-full">
          ...
          <div className="grid grid-cols-1 gap-1.5 sm:gap-2">
            {[
              ...
              <div key={stat} className="flex items-center gap-3 sm:gap-4 p-2 sm:p-2.5 rounded-lg sm:rounded-xl"
                style={{ background: 'rgba(15,142,77,0.1)', border: '1px solid rgba(74,222,128,0.2)' }}>
                <span className="text-[11px] sm:text-sm font-black text-green-400 w-16 sm:w-20 flex-shrink-0">{stat}</span>
                <span className="text-[10px] sm:text-xs text-white/60 leading-snug">{label}</span>
              </div>
            ))}
          </div>
        </div>
